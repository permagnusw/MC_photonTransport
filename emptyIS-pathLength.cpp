...
int main(){
	...
	R_vec[0] = R0;
	for(int i=1; i<N_R; i++){
		R_vec[i] = R_vec[i-1] + d_R;
	}

	for(int j=0; j<N_R; j++){
		double z_s = sqrt(R_vec[j]*R_vec[j] - r_e*r_e);
		...
		for(int k=0; k<N_avg; k++){
			...
			initPhotons(photons, R_vec[j], z_s, cos_theta0_s, a_s, b_s, G); //CHECK NA source
	...
	ofstream myFile;
	myFile.open("data/CoatedAl_final/105x105_rho90_t.txt");
